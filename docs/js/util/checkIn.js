...
export function initializeCheckIn() {
	...
	if (lastCheckInDate) {
		document.getElementById("last-check-in-date").textContent =
			"Last Check-In: " + new Date(lastCheckInDate).toISOString().split("T")[0];
	}

	if (checkInCount) {
		document.getElementById("check-in-count").textContent =
			"Count: " + checkInCount;
	}
}
